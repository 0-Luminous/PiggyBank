...
struct BankView: View {
    ...
    @State private var randomAmount: Int = 0
    @State private var showingAlert: Bool = false
    @State private var showingAddMoneyAlert = false
        ...
        NavigationStack {
            ...
            .fullScreenCover(isPresented: $showingAlert) {
                AddMoneyAlertView(
                    ...
                    onConfirm: {
                        addTransaction(amount: randomAmount)
                        showingAlert = false
                    }
                )
            }
    ...
    private func generateRandomAmount() {
        guard let min = Int(minAmount),
            let max = Int(maxAmount),
            min < max
        else { return }

        randomAmount = Int.random(in: min...max)
        showingAlert = true
    }

    private func addTransaction(amount: Int) {
        let currentDate = formatDate(Date())
        transactions.append(Transaction(date: currentDate, amount: amount))
    }
